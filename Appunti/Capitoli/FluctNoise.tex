...
\footnotetext{Nel limite per $ T \rightarrow \infty $ si intende che il supporto della funzione $ x(t) $ si allarghi, altrimenti tale limite sarebbe banalmente nullo.}
...
Dopo aver introdotto la densità spettrale per caratterizzare una variabile stocastica, si può esamnare un'altra quantità che da informazioni dirette sull'andamento temporale delle fluttuazioni, cioè la \textit{correlazione mediata sul tempo e sull'ensemble}, $ C(\tau) $.\vspace*{-10pt}
...
%		\item Caso puramente ondulatorio
%		\item Caratteristiche di coerenza
%		\item Interferometria
%		\begin{itemize}
%			\item Michelson
%			\item Hanbury Brown-Twiss
%		\end{itemize}
%	\end{itemize}
%	\item Rumore
%	\begin{itemize}
%		\item Teorema di Nyquist
%		\item Wiener-Khintchine
%		\item Nyquist microscopico
%		\item Shot noise
%	\end{itemize}
%\end{itemize}
...
\begin{tikzpicture}
	\genealogytree[template=signpost]{
		child{
			g{Fluttuazioni}
			c{Statistiche quantistiche}
			c{Caso puramente ondulatorio}
			c{Caratteristiche di coerenza}
			child{
				g{Interferometria}
				c{Michelson}
				c{Hanbury Brown-Twiss}
			}
		}
	}
	\genealogytree[tree offset=9cm]{
		child{
			g{Rumore}
			c{Teorema di Nyquist}
			c{Wiener-Khintchine}
			c{Nyquist microscopico}
			c{Shot noise}
		}
	}
\end{tikzpicture}
